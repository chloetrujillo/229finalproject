...
class SimpleCNN(nn.Module):
    ...
    def forward(self, x):
        return self.sigmoid(self.classifier(self.features(x)))


def evaluate(model, dataloader, device):
    model.eval()
    crop_correct = crop_off1 = crop_total = 0
    level_preds, level_labels = defaultdict(list), {}
    with torch.no_grad():
        for imgs, labels, level_ids in dataloader:
            imgs = imgs.to(device)
            outputs = model(imgs).squeeze(1)          # scalar in [0, 1]
            preds = (outputs * 9).round().long().clamp(0, 9)  # 0-indexed class
            crop_correct += (preds.cpu() == labels).sum().item()
            crop_off1 += ((preds.cpu() - labels).abs() <= 1).sum().item()
            crop_total += len(labels)
            for i in range(len(level_ids)):
                level_preds[level_ids[i]].append(outputs[i].item())
                level_labels[level_ids[i]] = labels[i].item()

    level_correct = level_off1 = 0
    for lid, pred_list in level_preds.items():
        pred = int(round(np.mean(pred_list) * 9))
    ...
    for epoch in range(args.epochs):
        ...
        with torch.no_grad():
            for imgs, labels, _ in val_dl:
                imgs, labels = imgs.to(device), labels.to(device)
                labels_norm = labels.float() / 9.0
                val_loss += criterion(model(imgs).squeeze(1), labels_norm).item() * len(labels)
