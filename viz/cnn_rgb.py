# 3-Channel RGB CNN with Ordinal Regression
# Same architecture as cnn_weighted.py but uses original RGB crops (3ch)
...
class LevelDatasetRGB(Dataset):
    """Loads RGB PNG crops. Pre-caches all to RAM on init."""
        ...
        if preload:
            ...
            for _, _, crop_infos in level_groups:
                for fn, _ in crop_infos:
                    all_files.add(fn)
            print(f"  Pre-loading {len(all_files)} RGB crops to RAM...", end=" ", flush=True)
            for fn in all_files:
                try:
                    img = np.array(Image.open(os.path.join(crops_dir, fn)), dtype=np.float32) / 255.0
                    if img.shape == (CROP_H, CROP_W, N_CHANNELS):
                        self.cache[fn] = img
                except Exception:
                    pass
            print("done.")
    ...
    def _load(self, fn):
        try:
            ...
            img = np.array(Image.open(os.path.join(self.crops_dir, fn)), dtype=np.float32) / 255.0
            if img.shape != (CROP_H, CROP_W, N_CHANNELS):
...
class MergedCropCNNRGB(nn.Module):
    """
    3-channel RGB CNN with ordinal regression (CORAL).

    Dimension flow for (3, 64, 386) input:
...
def main():
    ...
    train_ds = LevelDatasetRGB(train_levels, crops_dir, preload=True)
    val_ds = LevelDatasetRGB(val_levels, crops_dir, preload=True)
    test_ds = LevelDatasetRGB(test_levels, crops_dir, preload=True)
    ...
    model = MergedCropCNNRGB().to(device)
    ...
    for epoch in range(args.epochs):
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(),
                       os.path.join(PROJECT_ROOT, "models", "cnn_rgb.pth"))
        else:
            patience_counter += 1
    ...
    model.load_state_dict(torch.load(
        os.path.join(PROJECT_ROOT, "models", "cnn_rgb.pth"), weights_only=True))
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(PROJECT_ROOT, "models", "cnn_rgb_curves.png"), dpi=150)
    plt.close()
    print("Saved training curves to models/cnn_rgb_curves.png")
