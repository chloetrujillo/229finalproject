...
def analyze(model, val_dataloader, test_dataloader, out_dir):
    ...
    plt.close()
    print(f"Saved error analysis plot to {out_dir}/error_analysis.png")
    ...
    plt.close()
    print(f"Saved validation error analysis plot to {out_dir}/val_error_analysis.png")


def main(args):
    out_dir = "data_v2_v1_merge"
    ...
    selected_cols.extend(["x_range", "y_range"])
    ...
    if args.load_model:
        model.load_state_dict(torch.load("models/v2_v1_merge_model.pth"))
        print("Loaded model checkpoint from models/v2_v1_merge_model.pth")  
    ...
    for epoch in range(200):
        ...
        train_loss = 0.0
        for i, (x, y, _) in enumerate(train_dl_sel):
            optimizer.zero_grad()
            y_pred = model(x)
            loss = criterion(y_pred, y)
    ...
    torch.save(model.state_dict(), "models/v2_v1_merge_model.pth")
    print("Saved model checkpoint to models/v1_v2_merge_model.pth")
