# 6-Channel CNN with Ordinal Regression for predicting GD level difficulty
# Instead of RGB (3ch), each object type gets its own binary channel:
#   ch0: hazards, ch1: blocks, ch2: triggers, ch3: portals, ch4: orbs, ch5: unknown
# This prevents sparse features (hazards=0.2% of pixels) from being destroyed
# by CNN downsampling, since each type is in its own independent channel.
#
# Ordinal regression (CORAL cumulative threshold):
#   Output K-1=9 sigmoid logits for P(y>0), P(y>1), ..., P(y>8)
#   Label y → target = [1]*y + [0]*(K-1-y)
#   Loss = sum of K-1 binary cross-entropy losses
#   Prediction = count(sigmoid > 0.5)
...
N_CHANNELS = 6  # one per object type

# Object group colors (from render_levels.py) — RGB uint8
GROUP_COLORS_U8 = {
...
def rgb_to_6ch(img_u8):
    """Convert an RGB uint8 image to 6 binary channels (float32).
    img_u8: (H, W, 3) uint8
    Returns: (H, W, 6) float32 with values 0.0 or 1.0
    """
    h, w = img_u8.shape[:2]
    out = np.zeros((h, w, N_CHANNELS), dtype=np.float32)
    for i, name in enumerate(CHANNEL_NAMES):
        color = GROUP_COLORS_U8[name]
        # Exact match (our rendered images use exact colors)
...
class LevelDataset6Ch(Dataset):
    """Each item returns 6-channel merged crops for one level.
    Pre-caches all crops to RAM on init for fast training."""
    ...
    def __init__(self, level_groups, crops_dir, preload=True):
        ...
        if preload:
            # Pre-load and convert ALL crops to 6ch once
            all_files = set()
            for _, _, crop_infos in level_groups:
                for fn, _ in crop_infos:
                    all_files.add(fn)
            print(f"  Pre-loading {len(all_files)} crops to RAM...", end=" ", flush=True)
            for fn in all_files:
                img_u8 = np.array(Image.open(os.path.join(crops_dir, fn)))
                self.cache[fn] = rgb_to_6ch(img_u8)
            print("done.")

    def __len__(self):
        return len(self.levels)

    def _load_crop(self, fn):
        if fn in self.cache:
            return self.cache[fn]
        img_u8 = np.array(Image.open(os.path.join(self.crops_dir, fn)))
        return rgb_to_6ch(img_u8)

    def __getitem__(self, idx):
        level_id, stars, crop_infos = self.levels[idx]
        crop_infos = sorted(crop_infos, key=lambda ci: ci[1])

        imgs = [self._load_crop(fn) for fn, x in crop_infos]
        ...
        merged = np.stack(merged, axis=0)  # (M, 6, H, MERGED_W)
        label = int(stars) - 1
        return torch.from_numpy(merged), label


def collate_fn(batch):
    """Pad variable-length merged crops to max M in batch."""
...
class MergedCropCNN6Ch(nn.Module):
    ...
    Each channel = one object type (binary), so no information mixing.

    Ordinal output: K-1 = 9 logits for cumulative thresholds.
    ...
    def __init__(self, num_classes=10, feat_dim=256):
        ...
        # Ordinal regression: K-1 cumulative threshold logits
        self.ordinal_head = nn.Sequential(
            nn.Dropout(0.3),
            nn.Linear(feat_dim, num_classes - 1),  # 9 logits
        )
    ...
    def forward(self, merged, mask):
        ...
        merged: (B, M, 6, H, W)
        mask:   (B, M) bool
        Returns: ordinal logits (B, num_classes - 1)
        """
        ...
        return self.ordinal_head(x)  # (B, K-1)

    def predict(self, merged, mask):
        """Return predicted class indices (0..K-1)."""
        logits = self.forward(merged, mask)  # (B, K-1)
        probs = torch.sigmoid(logits)        # P(y > k)
        preds = (probs > 0.5).sum(dim=1)    # count thresholds exceeded
        return preds


def ordinal_loss(logits, labels, num_classes=10):
    """CORAL cumulative threshold loss.
    logits: (B, K-1) raw logits
    labels: (B,) integer labels 0..K-1
    Returns: scalar loss
    """
    K = num_classes
    # Build cumulative targets: label=y → [1]*y + [0]*(K-1-y)
...
def main():
    ...
    train_ds = LevelDataset6Ch(train_levels, crops_dir, preload=True)
    val_ds = LevelDataset6Ch(val_levels, crops_dir, preload=True)
    test_ds = LevelDataset6Ch(test_levels, crops_dir, preload=True)
    ...
    print(f"Loss: Ordinal regression (CORAL cumulative threshold)")
    ...
    for epoch in range(args.epochs):
        ...
        print(f"Epoch {epoch+1}/{args.epochs} | "
              ...
              f"Level Acc: {metrics['acc']:.4f} | Off1: {metrics['off1']:.4f} | "
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(),
                       os.path.join(PROJECT_ROOT, "models", "cnn_6ch.pth"))
        else:
            patience_counter += 1
    ...
    model.load_state_dict(torch.load(
        os.path.join(PROJECT_ROOT, "models", "cnn_6ch.pth"), weights_only=True))
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(PROJECT_ROOT, "models", "cnn_6ch_curves.png"), dpi=150)
    plt.close()
    print("Saved training curves to models/cnn_6ch_curves.png")
